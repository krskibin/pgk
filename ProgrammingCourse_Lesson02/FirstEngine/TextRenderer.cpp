...
void TextRenderer::Render( const std::vector< RenderableText >& texts )
{
	for( const auto& text : texts )
	{
		...
		m_font->DrawTextA( nullptr, text.text.c_str(), text.text.length(), &textRect, DT_CALCRECT, 0 );
		m_font->DrawTextA( nullptr, text.text.c_str(), text.text.length(), &textRect, DT_LEFT | DT_TOP, text.color );
	}
}
